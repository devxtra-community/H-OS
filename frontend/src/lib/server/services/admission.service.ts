...
export class AdmissionService {
  async createAdmission(data: {
    ...
    const result = await pool.query(
      `
      INSERT INTO admissions (id, patient_id, doctor_id, department_id, status)
  ...
  async requestDischarge(id: string) {
    await pool.query(
      `UPDATE admissions SET discharge_requested = true, discharge_requested_at = now() WHERE id = $1`,
      [id]
    );
  }
  ...
  async getDoctorAdmissions(doctorId: string) {
    const result = await pool.query(
      `
      SELECT a.id, a.patient_id, p.name AS patient_name, a.doctor_id, a.department_id, a.status, a.created_at, a.discharge_requested
      FROM admissions a
      LEFT JOIN patients p ON p.id = a.patient_id
      WHERE a.doctor_id = $1 AND a.status = 'ADMITTED'
  ...
  async getDischargeRequests() {
    const result = await pool.query(
      `
      SELECT a.id, a.patient_id, p.name AS patient_name, a.doctor_id
      FROM admissions a
      LEFT JOIN patients p ON p.id = a.patient_id
      WHERE a.discharge_requested = true AND a.status = 'ADMITTED'
      ORDER BY a.created_at ASC
      `
    );
    return result.rows;
  ...
  async getCurrentAdmission(patientId: string) {
    const result = await pool.query(
      `SELECT * FROM admissions WHERE patient_id = $1 AND status != 'DISCHARGED' LIMIT 1`,
      [patientId]
    );
